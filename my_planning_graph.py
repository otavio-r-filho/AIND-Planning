...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        # TODO refine names in this function
        ...
        for action in self.all_actions:
            ...
            if all(precond in s_level for precond in new_node_a.prenodes):
                action_level.add(new_node_a)
    ...
    def add_literal_level(self, level):
        ...
        pre_s_level = self.s_levels[level - 1]
        pre_a_level = self.a_levels[level - 1]
        new_state_level = set()
        
        # All s_nodes from previous level should be added because of the persistence actions
        for pre_s_node in pre_s_level:
            new_state_level.add(pre_s_node)

        for action in pre_a_level:
            for effect in action.effnodes:
                new_state_level.add(effect)
        
        self.s_levels.append(new_state_level)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if any(add_effect in act2.effect_rem for add_effect in act1.effect_add) or \
           any(rem_effect in act2.effect_add for rem_effect in act1.effect_rem):
            is_mutex = True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if any(precond_pos in act2.effect_rem for precond_pos in act1.precond_pos) or \
           any(precond_neg in act2.effect_add for precond_neg in act1.precond_neg) or \
           any(precond_pos in act1.effect_rem for precond_pos in act2.precond_pos) or \
           any(precond_neg in act1.effect_add for precond_neg in act2.precond_neg):
            is_mutex = True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # if any(precond_pos in act2.precond_neg for precond_pos in act1.precond_pos) or \
        #    any(precond_neg in act2.precond_pos for precond_neg in act1.precond_neg):
        #     is_mutex = True

        for parent1 in node_a1.parents:
            for parent2 in node_a2.parents:
                if parent1 in parent2.mutex:
                    is_mutex = True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent_s1 in node_s1.parents:
            if any(mutex in node_s2.parents for mutex in parent_s1.mutex):
                is_mutex = True
    ...
    def h_levelsum(self) -> int:
        ...
        goals_set = set(self.problem.goal)

        for level in range(len(self.s_levels)):
            for node_s in self.s_levels[level]:
                if node_s.symbol in goals_set and \
                   node_s.is_pos:
                    goals_set.remove(node_s.symbol)
                    level_sum += level
                    break
